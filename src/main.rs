...
fn show_error(message: &str) {
  ...
  unsafe {
    MessageBoxW(HWND::default(), message_w.as_ptr(), title.as_ptr(), MB_OK | MB_ICONERROR);
  }
}
...
fn expand_env_windows(value: &str) -> Result<String, String> {
  ...
  if needed == 0 {
    return Err(format!("ExpandEnvironmentStringsW failed for value: {} (error {})", value, unsafe { GetLastError() }));
  }
...
fn main() {
  ...
  let application_name = if argument.is_empty() { program_w.as_ptr() } else { null() };
  let command_line = if argument.is_empty() { null_mut() } else { cmd_line_w.as_mut_ptr() };
